...
def to_the_right_of_deic(a, b):
    ...
    weighted_measure = 0.9 * (0.4 * horizontal_component + 0.6 * vertical_component) #+ 0.1 * distance_factor

    disp_vec = np.array(a.bbox_centroid - b.bbox_centroid)
    dist = np.linalg.norm(disp_vec)
    disp_vec = disp_vec / dist
    extr_right = np.array([1, 0, 0])
    cos = extr_right.dot(disp_vec)
    extr_component = math.e ** (- 0.1 * (1 - cos)) * math.e ** (- 0.05 * dist / max(a.size, b.size))
    #print ("RIGHT OF EXTR COMPONENT", extr_component, weighted_measure)
    return max(weighted_measure, extr_component)
